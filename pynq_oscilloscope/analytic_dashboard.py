"""
pynq_oscilloscope.analytic_dashboard: Real-Time Diagnostic Dashboard for Acoustic Analytics.
Renders synchronous Amplitude vs. Time (Hilbert Envelopes, ILD) and Frequency vs. Time (STFT Spectrogram, Sub-Hertz Pitch).
"""
...
class AcousticAnalyticDashboard:
    """
    Real-Time Dual-Engine Acoustic Diagnostic Dashboard.
    Provides live Hilbert envelope tracking, Short-Time Energy, rolling STFT spectrogram,
    and microsecond-level inter-channel phase difference monitoring.
    """
    ...
    def _build_ui(self):
        ...
        self.readout_metrics = widgets.HTML("<span style='color:#00FFCC; font-family:monospace; font-size:13px; font-weight:bold;'>A0: Vpp=0.00V | A1: Vpp=0.00V | f0=0.0Hz | ΔL=0.0dB | Δϕ=0.0°</span>")
    ...
    def _build_plots(self):
        ...
        # Tab 1: Amplitude & Energy vs. Time
        self.fig_amp_time = make_subplots(
            rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12,
            subplot_titles=("<b>Instantaneous Voltages & Hilbert Envelopes (A0 & A1)</b>", "<b>Inter-aural Level Difference ΔL(t) [Energy Ratio in dB]</b>")
        )
        self.fig_amp_time = go.FigureWidget(self.fig_amp_time)
        self.fig_amp_time.add_scatter(x=t_ms, y=[1.65]*len(t_ms), mode="lines", line=dict(color="rgba(0, 255, 204, 0.4)", width=1.2), name="A0 Raw (V)", row=1, col=1)
        self.fig_amp_time.add_scatter(x=t_ms, y=[0.0]*len(t_ms), mode="lines", line=dict(color="#00FFCC", width=2.2), name="A0 Hilbert Env A(t)", row=1, col=1)
        self.fig_amp_time.add_scatter(x=t_ms, y=[1.65]*len(t_ms), mode="lines", line=dict(color="rgba(255, 0, 127, 0.4)", width=1.2), name="A1 Raw (V)", row=1, col=1)
        self.fig_amp_time.add_scatter(x=t_ms, y=[0.0]*len(t_ms), mode="lines", line=dict(color="#FF007F", width=2.2), name="A1 Hilbert Env A(t)", row=1, col=1)
        self.fig_amp_time.add_scatter(x=t_ms, y=[0.0]*len(t_ms), mode="lines", line=dict(color="#FFA500", width=1.8), name="ILD ΔL(t) [dB]", row=2, col=1)
        self.fig_amp_time.update_layout(template="plotly_dark", height=520, margin=dict(l=40, r=20, t=45, b=35), uirevision="t1")
        self.fig_amp_time.update_yaxes(range=[0, 3.3], title="Voltage (V)", row=1, col=1)
        self.fig_amp_time.update_yaxes(range=[-30, 30], title="Level Diff (dB)", row=2, col=1)
        self.fig_amp_time.update_xaxes(range=[0, self.total_duration_ms], title="Time (Milliseconds)", row=2, col=1)

        # Tab 2: Frequency vs. Time & Spectrogram
        self.fig_freq_time = make_subplots(
            rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12,
            subplot_titles=("<b>Rolling STFT Spectrogram (Channel 1 / A0)</b>", "<b>Sub-Hertz Fundamental Pitch f0(t) & Phase Difference Δϕ(t)</b>")
        )
        self.fig_freq_time = go.FigureWidget(self.fig_freq_time)
        dummy_z = np.full((128, 30), -90.0)
        self.fig_freq_time.add_heatmap(z=dummy_z, x=np.linspace(0, self.total_duration_ms, 30), y=initial_freq, colorscale="Viridis", zmin=-80, zmax=0, colorbar=dict(title="dBV", len=0.45, y=0.8), row=1, col=1)
        self.fig_freq_time.add_scatter(x=[0, self.total_duration_ms], y=[1000, 1000], mode="lines+markers", line=dict(color="#00FFCC", width=2.0), name="Pitch f0(t) [Hz]", row=2, col=1)
        self.fig_freq_time.add_scatter(x=[0, self.total_duration_ms], y=[0, 0], mode="lines", line=dict(color="#FFA500", width=1.8, dash="dash"), name="Phase Diff Δϕ(t) [°]", row=2, col=1)
        self.fig_freq_time.update_layout(template="plotly_dark", height=540, margin=dict(l=40, r=20, t=45, b=35), uirevision="t2")
        self.fig_freq_time.update_yaxes(range=[0, self.fs_per_ch / 2.0], title="Frequency (Hz)", row=1, col=1)
        self.fig_freq_time.update_yaxes(range=[-180, 180], title="Degrees / Hz", row=2, col=1)
        self.fig_freq_time.update_xaxes(range=[0, self.total_duration_ms], title="Time (Milliseconds)", row=2, col=1)

        # Tab 3: Combined View
        self.fig_combined = make_subplots(
            rows=2, cols=1, shared_xaxes=True, vertical_spacing=0.12,
            subplot_titles=("<b>Hilbert Amplitude Envelopes (A0 & A1)</b>", "<b>Channel 1 Spectrogram Waterfall</b>")
        )
        self.fig_combined = go.FigureWidget(self.fig_combined)
        self.fig_combined.add_scatter(x=t_ms, y=[0.0]*len(t_ms), mode="lines", line=dict(color="#00FFCC", width=2.0), name="A0 Env", row=1, col=1)
        self.fig_combined.add_scatter(x=t_ms, y=[0.0]*len(t_ms), mode="lines", line=dict(color="#FF007F", width=2.0), name="A1 Env", row=1, col=1)
        self.fig_combined.add_heatmap(z=dummy_z, x=np.linspace(0, self.total_duration_ms, 30), y=initial_freq, colorscale="Viridis", zmin=-80, zmax=0, showscale=False, row=2, col=1)
        self.fig_combined.update_layout(template="plotly_dark", height=520, margin=dict(l=40, r=20, t=45, b=35), uirevision="t3")
        self.fig_combined.update_yaxes(range=[0, 1.8], title="Envelope (V)", row=1, col=1)
        self.fig_combined.update_yaxes(range=[0, self.fs_per_ch / 2.0], title="Frequency (Hz)", row=2, col=1)
        self.fig_combined.update_xaxes(range=[0, self.total_duration_ms], title="Time (Milliseconds)", row=2, col=1)

        self.tabs = widgets.Tab(children=[self.fig_amp_time, self.fig_freq_time, self.fig_combined])
        self.tabs.set_title(0, "📈 Amplitude & Energy vs. Time")
        self.tabs.set_title(1, "📊 Frequency vs. Time (Spectrogram)")
        self.tabs.set_title(2, "🔀 Multi-Domain Analytic View")
    ...
    def _on_profile_change(self, change):
        if self.overlay and hasattr(self.overlay, "set_profile"):
            ...
            print(f"[Analytics] Switched regime to '{change['new']}' ({self.fs_per_ch} Hz, {self.total_duration_ms:.2f} ms window)")
    ...
    def _update_loop(self):
        ...
        dma_armed = False
        print(f"[AcousticAnalytics] Diagnostic Engine Active ({self.fs_per_ch:.0f} Hz Synchronous Stream)")
        ...
        try:
            while self._is_running:
                ...
                if dma_time.recvchannel.idle:
                    ...
                    # 1. Analytic Amplitude Envelopes
                    ...
                    # 3. Sliding STFT Spectrogram (Ch1 / A0)
                    ...
                    # 4. Instantaneous Phase Difference (Delta_phi) & Fundamental Pitch (f0)
                    phase_diff_deg = AcousticAnalytics.compute_instantaneous_phase_diff(p_v1, p_v2)
                    mean_phase_deg = float(np.median(phase_diff_deg))

                    # Frequency peak detection on full frame
                    spec_full = 20.0 * np.log10(np.maximum(np.abs(np.fft.rfft(p_v1 - np.mean(p_v1))) / (n_pts / 2.0), 1e-6))
                    ...
                    if active_tab == 0:  # Tab 1: Amplitude vs. Time
                        with self.fig_amp_time.batch_update():
                            ...
                            self.fig_amp_time.data[1].y = env_a0
                            self.fig_amp_time.data[2].x = t_ms
                            self.fig_amp_time.data[2].y = p_v2
                            self.fig_amp_time.data[3].x = t_ms
                            self.fig_amp_time.data[3].y = env_a1
                            self.fig_amp_time.data[4].x = t_ild_ms
                            self.fig_amp_time.data[4].y = ild_db
                            self.fig_amp_time.layout.xaxis2.range = [0, t_ms[-1]]

                    elif active_tab == 1:  # Tab 2: Frequency vs. Time
                        with self.fig_freq_time.batch_update():
                            self.fig_freq_time.data[0].z = spec_mat
                            self.fig_freq_time.data[0].x = stft_times
                            self.fig_freq_time.data[0].y = stft_freqs
                            self.fig_freq_time.data[1].x = [0, t_ms[-1]]
                            self.fig_freq_time.data[1].y = [p_f0, p_f0]
                            self.fig_freq_time.data[1].text = [f"f0 = {p_f0:.1f} Hz"]
                            self.fig_freq_time.data[2].x = t_ms
                            self.fig_freq_time.data[2].y = phase_diff_deg
                            self.fig_freq_time.layout.xaxis2.range = [0, t_ms[-1]]

                    elif active_tab == 2:  # Tab 3: Combined View
                        with self.fig_combined.batch_update():
                            self.fig_combined.data[0].x = t_ms
                            self.fig_combined.data[0].y = env_a0
                            self.fig_combined.data[1].x = t_ms
                            self.fig_combined.data[1].y = env_a1
                            self.fig_combined.data[2].z = spec_mat
                            self.fig_combined.data[2].x = stft_times
                            self.fig_combined.data[2].y = stft_freqs
                            self.fig_combined.layout.xaxis2.range = [0, t_ms[-1]]

                    self.readout_metrics.value = (
                        f"<span style='color:#00FFCC; font-family:monospace; font-size:13px; font-weight:bold;'>"
                        f"A0: Vpp={vpp1:.2f}V | A1: Vpp={vpp2:.2f}V | f0={p_f0:.1f}Hz | ΔL={mean_ild:+.1f}dB | Δϕ={mean_phase_deg:+.1f}°"
                        f"</span>"
                    )
